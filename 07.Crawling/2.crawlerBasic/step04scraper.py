...
3. 의도 및 구현 방식
    ...
    3. process
        ...
        호출 순서: fetch - scrape - save --> 정형 process
        개발 권장 방식: 순서에 맞게 호출하는 별도의 함수 개발 --> main()
...
# 크롤링(웹 페이지를 다운로드 하는 작업)
# 웹페이지 추출하기 : fetch()
# 웹 페이지 인코딩 형식은 추출한 인코딩 정보를 기반으로 추출
def fetch(url):  
    f = urlopen(url)   # url에 해당하는 웹 페이지를 통으로 긁어옴
    
    # HTTP 헤더를 기반으로 인코딩 형식을 추출
    encoding = f.info().get_content_charset(failobj="utf-8")
    # print("--- 1. 인코딩 정보 : ", encoding)   # utf-8
    
    # 추출한 인코딩 형식을 기반으로 문자열을 디코딩, 디코딩 안 할 경우 한글 깨짐 현상 발생 가능
    # 몇 사이트는 encoding 정보가 euc-kr로 되어 있기도 함. 해당 사이트가 구현되어 있는 정보로 디코딩 수행
...
def scrape(html):
    ...
    for data in re.findall(r'<td class="left"><a href="(.*?)">(.*?)</a></td>', html):
        url = 'http://www.hanbit.co.kr' + data[0]
        title = data[1]
...
def save(books):
    ...
    # 커서를 추출
    cur = con.cursor()
    # cur.execute('''DROP TABLE books''')   에러 발생 시 다음 로직 실행 불가
    # books 테이블을 생성 - 
    cur.execute('''
        CREATE TABLE books (
            title varchar2(200),
